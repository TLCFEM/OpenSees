...
int LeeSparse::add_current_stiffness(const Matrix& EK, const ID& EI, const double S) {
	if(1. == S) { for(auto I = 0; I < EI.Size(); ++I) if(EI(I) >= 0) for(auto J = 0; J < EI.Size(); ++J) if(EI(J) >= 0) current_stiffness.at(EI(I), EI(J)) = EK(I, J); }
	else for(auto I = 0; I < EI.Size(); ++I) if(EI(I) >= 0) for(auto J = 0; J < EI.Size(); ++J) if(EI(J) >= 0) current_stiffness.at(EI(I), EI(J)) = S * EK(I, J);

	return 0;
}

int LeeSparse::add_stiffness(const Matrix& EK, const ID& EI, const double S) {
	if(1. == S) { for(auto I = 0; I < EI.Size(); ++I) if(EI(I) >= 0) for(auto J = 0; J < EI.Size(); ++J) if(EI(J) >= 0) stiffness.at(EI(I), EI(J)) = EK(I, J); }
	else for(auto I = 0; I < EI.Size(); ++I) if(EI(I) >= 0) for(auto J = 0; J < EI.Size(); ++J) if(EI(J) >= 0) stiffness.at(EI(I), EI(J)) = S * EK(I, J);

	return 0;
}

int LeeSparse::add_damping(const Matrix& EC, const ID& EI, const double S) {
	if(1. == S) { for(auto I = 0; I < EI.Size(); ++I) if(EI(I) >= 0) for(auto J = 0; J < EI.Size(); ++J) if(EI(J) >= 0) damping.at(EI(I), EI(J)) = EC(I, J); }
	else for(auto I = 0; I < EI.Size(); ++I) if(EI(I) >= 0) for(auto J = 0; J < EI.Size(); ++J) if(EI(J) >= 0) damping.at(EI(I), EI(J)) = S * EC(I, J);

	return 0;
}

int LeeSparse::add_mass(const Matrix& EM, const ID& EI, const double S) {
	if(1. == S) { for(auto I = 0; I < EI.Size(); ++I) if(EI(I) >= 0) for(auto J = 0; J < EI.Size(); ++J) if(EI(J) >= 0) mass.at(EI(I), EI(J)) = EM(I, J); }
	else for(auto I = 0; I < EI.Size(); ++I) if(EI(I) >= 0) for(auto J = 0; J < EI.Size(); ++J) if(EI(J) >= 0) mass.at(EI(I), EI(J)) = S * EM(I, J);
